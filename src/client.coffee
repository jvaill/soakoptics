...
class Client
  ...
  filePaths: (type) =>
    ...
    htm = path.resolve("#{BROWSERSHOTS_PATH}/html/#{md5}.htm")
    pngFull = path.resolve("#{BROWSERSHOTS_PATH}/png-full/#{pngFilename}")
    pngThumb = path.resolve("#{BROWSERSHOTS_PATH}/png-thumb/#{pngFilename}")
